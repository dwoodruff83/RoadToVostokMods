...
- **MCP servers** (`.mcp.json` at workspace root, mirrored to `.vscode/mcp.json` for VS Code MCP-aware extensions):
  - `godotlens` — wraps Godot's built-in LSP via [godotlens-mcp](https://github.com/pzalutski-pixel/godotlens-mcp). Gives compiler-accurate `gdscript_definition` / `gdscript_references` / `gdscript_symbols` etc. across whatever project is loaded in the running Godot Editor. Best target: open `reference/RTV_decompiled/project.godot` so vanilla code is queryable. **Path-format gotcha**: tools want absolute filesystem paths (e.g. `f:/RoadToVostokMods/reference/RTV_decompiled/Scripts/Furniture.gd`). `res://...` URIs and workspace-relative paths return `No results`. All line/character coordinates are zero-based (editor line 1 = pass line 0).
