#!/usr/bin/env python3
"""Package the Cat Auto Feed mod into a .vmz archive.

Usage:
    python build.py                 # builds CatAutoFeed.vmz next to this script
...
MOD_FILES = ["Main.gd", "config.gd", "Logger.gd", "BowlPickup.gd", "DatabaseInject.gd", "Cat_Bowl.tres", "Cat_Bowl_2x2.tscn", "Cat_Bowl.tscn"]
ASSET_DIRS = ["assets"]
...
def build(src_dir: Path, out_path: Path) -> None:
    ...
    with zipfile.ZipFile(out_path, "w", zipfile.ZIP_DEFLATED) as z:
        ...
        # .godot/imported/ holds compiled textures and meshes Godot generates
        # at import time. Shipping them means the game doesn't need to reimport
        # at mod-load (and can't, since the runtime has no import pipeline).
